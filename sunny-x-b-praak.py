...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def starter_strategy(self, game_state, turn_string, game_map):
        ...
        if game_state.turn_number % 3 == 0 and game_state.turn_number > 6:
            ATTACK_STATUS = 1
        else:
            ATTACK_STATUS = 0
        ...
        if game_state.turn_number == 3 or game_state.turn_number == 6:
                if not self.detect_billert(game_state, left_billert):
                    my_empty_edges = self.filter_blocked_locations([[0,13]], game_state)
                    gamelib.debug_write("Deploy edges", my_empty_edges)
                    path = self.attack(game_state, turn_string, my_empty_edges)
                    self.build_support(game_state, path)
                elif not self.detect_billert(game_state, right_billert):
                    my_empty_edges = self.filter_blocked_locations([[27,13]], game_state)
                    gamelib.debug_write("Deploy edges", my_empty_edges)
                    path = self.attack(game_state, turn_string, my_empty_edges)
                    self.build_support(game_state, path)
                else:
        ...
        if enemy_health <= 10 and int(game_state.get_resource(MP)) > 3 and int(game_state.get_resource(MP)) < 8:
            for i in intial_queue:
                            game_state.attempt_remove(i[0])
            for j in priority_queue:
                            game_state.attempt_remove(j[0])
        elif enemy_health <= 10 and int(game_state.get_resource(MP)) >= 8:
            if not self.detect_corner(game_state, left_array):
                self.finishing_punch(game_state, left_support)
                game_state.attempt_spawn(SCOUT, [14, 0], 1000)
            elif not self.detect_corner(game_state, right_array):
                self.finishing_punch(game_state, right_support)
    ...
    global center_edges
    center_edges = [[3, 10], [4, 9], 
                        [5, 8], [6, 7], [7, 6], [8, 5], [9, 4], 
                        [24, 10], [23, 9], [22, 8], [21, 7], [20, 6], [19, 5], 
                        [18, 4]]
    global left_array, right_array
    left_array = [[0, 14], [1, 14]]
    right_array = [[26, 14], [27, 14]]
    def detect_corner(self, game_state, array):
        for i in array:
    ...
    global left_support, right_support, left_billert, right_billert
    left_support = [[2, 13], [3, 12], [4, 11], [5, 10], [6, 9], [7, 8], [8, 7],
                     [9, 6], [10, 5], [11, 4], [12, 3], [13, 2], [14, 1]]
    right_support = [[25, 13], [24, 12], [23, 11], [22, 10], [21, 9], [20, 8], 
                     [19, 7], [18, 6], [17, 5], [16, 4], [15, 3], [14, 2], [13, 1]]
    
    left_billert = [[3, 15], [4, 15]]
    right_billert = [[23, 15], [24, 15]]

    def detect_billert(self, game_state, array):
        for i in array:
            unit = game_state.contains_stationary_unit(i)
            if unit:
                gamelib.debug_write("Unit Lol", unit)
                gamelib.debug_write("Upgrade Lmao", unit.upgrade())
                bill = unit.unit_type == 'DF' and unit.upgrade()
                if not bill:
                    return False
        return True
    
    def finishing_punch(self, game_state, main_supports):
        for k in main_supports:
            game_state.attempt_spawn(SUPPORT, k)
        for l in main_supports:
    ...
    def get_not_healthy(self, turn_string):
        state = json.loads(turn_string)
        units = state["p1Units"]
        unit_information = self.config["unitInformation"]  # Access unit configuration
        low_health_locations = []
        for i, unit_list in enumerate(units):  # Each i corresponds to a specific unit type
            unit_type = unit_information[i]["shorthand"]
            for unit in unit_list:
                x, y, health, unit_id = unit
                #units_with_type.append([x, y, health, unit_id, unit_type])
                if unit_type == 'DF' and health < 0.25 * 75 and health > 0:
                    low_health_locations.append([x,y])
        return low_health_locations
    ...
    def attack(self, game_state, turn_string, edges):
        my_empty_edges = self.filter_blocked_locations(edges, game_state)
        locations = self.least_damage_modified(game_state, my_empty_edges)
        gamelib.debug_write("Min Damages", locations)
        healths = []
        for i in locations:
            gamelib.debug_write("Location", [i])
            healths.append(self.max_damage_scout(game_state, [i]))
        
        path = locations[healths.index(min(healths))]
    ...
    def build_defences(self, game_state, turn_string, defence_list):
        ...
        low_health_locations = self.get_not_healthy(turn_string)
        for i in defence_list:
            if i[1] == "WALL":
                ...
                if i[0] not in low_health_locations:
    ...
    def least_damage_modified(self, game_state, location_options):
        """
        This function helps us guess which location is the safest to spawn moving units from.
        It gets the path the unit will take, then checks locations on that path to 
        estimate the path's damage risk.
        """
        damages = []
        # Get the damage estimate for each path
        for location in location_options:
            path = game_state.find_path_to_edge(location, game_state.get_target_edge(location))
            damage = 0
            for path_location in path:
                # Get the number of enemy turrets that can attack each location and multiply by turret damage
                damage += len(game_state.get_attackers(path_location, 0)) * gamelib.GameUnit(TURRET, game_state.config).damage_i
            damages.append(damage)
        
        # Find the minimum damage value
        min_damage = min(damages)
        
        # Get all locations with the minimum damage
        min_damage_locations = [location for location, damage in zip(location_options, damages) if damage == min_damage]
        
        return min_damage_locations
    
    def max_damage_scout(self, game_state, unit_spawn_location_options):
        paths = self.least_damage_spawn_path(game_state, unit_spawn_location_options)
        health = 0
        for path in paths:
            scout = gamelib.GameUnit(SCOUT, self.config, player_index=0, health= 12, x=path[0], y=path[1])
            target_unit = game_state.get_target(scout)
            gamelib.debug_write("Target unit", target_unit)
            if target_unit:
                target_health = target_unit.health
                if target_unit.unit_type == 'DF':
